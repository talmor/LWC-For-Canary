...
public class Command_Admin implements LWC_Command {
	public void execute(LWC lwc, Player player, String[] args) {
		...
		if (action.equals("report")) {
			...
			Performance
					.setPlayersOnline(etc.getServer().getPlayerList().size());
			...
			if (updated) {
				...
				player.sendMessage("§2Updated LWC successfully to version: "
						+ lwc.getUpdater().getLatestVersion());
			} else {
			...
			if (args.length < 3) {
				lwc.sendSimpleUsage(player,
						"/lwc -admin limits <count> <Group/User>");
				return;
			}
			...
			for (int i = 3; i < args.length; i++) {
				...
				if (limit != -2) {
					lwc.getPhysicalDatabase().registerProtectionLimit(
							isGroup ? 0 : 1, limit, entity);
					player.sendMessage("§2Registered limit of §6" + limit
							+ "§2" + " chests to the "
							+ (isGroup ? "group" : "user") + " " + "§6"
							+ entity);
				} else {
					lwc.getPhysicalDatabase().unregisterProtectionLimit(
							isGroup ? 0 : 1, entity);
			...
			if (args.length < 2) {
				lwc.sendSimpleUsage(player,
						"/lwc -admin clear chests|limits|rights");
				return;
			}
	...
	public boolean validate(LWC lwc, Player player, String[] args) {
		return (lwc.isAdmin(player))
				&& ((StringUtils.hasFlag(args, "a")) || (StringUtils.hasFlag(
						args, "admin")));
	}
}
 ...
 * Qualified Name: Command_Admin JD-Core Version: 0.6.0
 */