...
public class LWC extends Plugin {
	...
	public boolean canAccessChest(Player player, Entity chest) {
		...
		case 2:
			...
			if (!player.getName().equalsIgnoreCase(chest.getOwner()))
				if ((instance.getPrivateAccess(1, chest.getID(),
						new String[] { player.getName() }) == -1)
						&& (instance.getPrivateAccess(0, chest.getID(),
								player.getGroups()) == -1))
					return false;
			return true;
		}
	...
	public boolean canAccessChest(Player player, int x, int y, int z) {
		...
		return canAccessChest(player,
				this.physicalDatabase.loadProtectedEntity(world, x, y, z));
	}
	...
	public void enable() {
		try {
			...
			/*
			 * this.updater.check(); this.updater.update();
			 * 
			 * if ((ConfigValues.AUTO_UPDATE.getBool()) &&
			 * (this.updater.checkDist())) { log("Reloading LWC");
			 * etc.getLoader().reloadPlugin("LWC"); return; }
			 */
	...
	public boolean enforceChestLimits(Player player) {
		...
		if (userLimit != -1) {
			...
			String groupName = player.getGroups().length > 0 ? player
					.getGroups()[0] : etc.getInstance().getDefaultGroup().Name;
			...
			for (String group : inheritedGroups) {
				...
				if (groupLimit != -1) {
					int chests = this.physicalDatabase.getChestCount(player
							.getName());
	...
	public List<ComplexBlock> getEntitySet(World world, int x, int y, int z) {
		...
		while (true) {
			...
			ComplexBlock block = world.getComplexBlock(x + (isXDir ? dev : 0),
					y, z + (isXDir ? 0 : dev));
	...
	public int getPlayerDropTransferTarget(String player) {
		String rawTarget = this.memoryDatabase.getModeData(player,
				"dropTransfer");
		try {
	...
	public boolean isPlayerDropTransferring(String player) {
		return (this.memoryDatabase.hasMode(player, "dropTransfer"))
				&& (this.memoryDatabase.getModeData(player, "dropTransfer")
						.startsWith("t"));
	}
	...
	public boolean isInCuboidSafeZone(Player player) {
		...
		try {
			...
			Class cuboidClass = cuboidPlugin.getClass().getClassLoader()
					.loadClass("CuboidAreas");

			Method findCuboidArea = cuboidClass.getMethod("findCuboidArea",
					new Class[] { Integer.TYPE, Integer.TYPE, Integer.TYPE });

			Object cuboidC = findCuboidArea.invoke(
					null,
					new Object[] { Integer.valueOf((int) player.getX()),
							Integer.valueOf((int) player.getY()),
			...
			Class cuboidPluginClass = cuboidPlugin.getClass().getClassLoader()
					.loadClass("CuboidPlugin");

			Field globalDisablePvP = cuboidPluginClass
					.getDeclaredField("globalDisablePvP");
	...
	private List<ComplexBlock> _validateChest(List<ComplexBlock> entities,
			ComplexBlock block) {
		if (block == null) {
			return entities;
		}
 ...
 * Qualified Name: LWC JD-Core Version: 0.6.0
 */